        ...
        for file_name in all_files:
            ...
            try:
                ...
                if file_name.endswith(('.docx', '.pptx','.html', '.htm')):
                    ...
                    print(f"📖 正在处理文件 [{file_name}] 的 {len(chunks)} 个片段...")

                    collection.upsert(
                        ids=[f"text_{file_name}_{j}" for j in range(len(chunks))],
                        documents=chunks,
                ...
                # 情况 B: Excel 表格
                    ...
                    for sheet_name, df in df_dict.items():
                        text = text +(f"--- 表格: {sheet_name} ---\n")
                        text = text +(df.fillna("").to_markdown(index=False) + "\n\n")
                ...
                # 情况 C: 图片 OCR
                    ...
                    try:
                        ...
                        print(f"🖼️ 正在编码图片视觉特征: {file_path}")
                        collection.upsert(
                            ids=[f"img_{file_name}"],
                            images=[img_array],  # 关键修复：提供图像数据
                            uris=[file_path],  # 依然保存路径以便后续展示
                            metadatas=[{"type": "image", "source": file_name}]
                        )
                    except Exception as e:
                        print(f"❌ 图像 {file_name} 加载失败: {e}")
